...
def get_todo(inputs,conf, out_path):
    completed = [ i for i in os.listdir(args.out_path) if i.endswith('_timeseries.tsv')]
    completed = [i for i in map(replace_file_ext,completed)]
    df = pd.DataFrame([inputs,conf],index = ['inputs','conf']).transpose()
    todo = df[~df['inputs'].isin(completed)
...
def get_batches(n_jobs, files, conf_files):
    n_per_job = int(len(files)/n_jobs)
    batches = []
    batches_conf = []
    for i in range(0,len(files),n_per_job):
        if i == (n_jobs - 1)*n_per_job:
            batches.append(files[i:])
            batches_conf.append(conf_files[i:])
            break
        batches.append(files[i:(i + n_per_job)])
        batches_conf.append(conf_files[i:(i + n_per_job)])
...
def make_sh(account,time,mem,n_jobs,out_path):
    ...
    with open('submit_template.sh', 'r') as f:
        src = Template(f.read())
